...
<https://mlr3book.mlr-org.com/index.html>, in particular the first two sections
"Introduction and Overview" and "Basics". You can open a separate R script
(ctrl-shift-n) and experiment with the code from the book yourself.
...
gallery** at <https://mlr3gallery.mlr-org.com/>.
...
-   random forest (rf): <https://en.wikipedia.org/wiki/Support-vector_machine>
-   extreme gradient boosting (xgb): <https://en.wikipedia.org/wiki/XGBoost>
...
```{r}
learn_ids <- setdiff(obs_ids, test_ids)
```
...
-   In particular the first five features as they clearly show a different
    (empirical) distribution in the `malignant` and `benign` samples.
...
-   Because information from the test data might leak, i.e. influence our model
    development (and particular) selection process such that the final
    performance assessment on the test data will potentially be biased.
...
-   The features `bare_nuclei` and `cl_thickness` are strong predictors as they
    show a significant association with the target variable (at the
    $\alpha=0.05$ level).
...
classif_metrics <- function(pred,
  ...
  out <-
    ...
    mutate(
      ...
      tnr = mean(y[neg]),
      tnr_se = sqrt(tnr*(1-tnr)/n0),
      tnr_lower = tnr - cv*tnr_se,
      tnr_upper = tnr + cv*tnr_se,
...
-   For estimated proportions of one, we have `se=0` and thus a zero width
    confidence interval.
-   Upper values of \> 1 are possible for approximate confidence intervals
-   Both issues can be resolved by using more complex inference methods such as
    the bootstrap (we'll have a look of this method later).
...
-   The maximum number of training runs. In case of the `"random_search"` tuning
    strategy, this is equal to the evaluated number of hyperparameters.
...
-   The confidence intervals for `bacc` clearly overlap, indicating no
...
bacc_delta <- function(d, positive = "malignant"){
  ...
  bacc1 <- 0.5 * mean(y1[pos]) + 0.5 * mean(y1[neg])
  bacc2 <- 0.5 * mean(y2[pos]) + 0.5 * mean(y2[neg])
  delta <- bacc1 - bacc2
...
-   The data does not indicate that there is a significance difference at the
    $\alpha=0.05$ level with regard to balanced accuracy between the baseline
    logistic regression model and the best SVM model.
...
rf_roc <- roc(response = rf_pred_pr$truth,
              predictor = rf_pred_pr$prob[, "malignant"],
              levels = c("benign", "malignant")) 
```
...
-   There is a small empirical difference in AUCs in favor of the (simpler)
    logistic regression model. At the $\alpha = 0.05$ level this difference is
